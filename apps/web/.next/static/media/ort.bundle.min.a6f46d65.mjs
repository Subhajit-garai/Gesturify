...
`}
      ...
      }`}},{inputs:[p],outputs:[d?-1:0]})[0],d&&i.compute(tU(x,c),{inputs:[x]})},nc=e=>t$({axis:e.axis})}),sw=q(()=>{"use strict";nk(),nI(),nB(),nh=e=>Array.from(e.getBigInt64Array(),Number),nf=e=>{var r;let i,a,n,s,o,u,l;(e=>{if(!e||2!==e.length)throw Error("Tile requires 2 inputs.");if(1!==e[0].dataType&&10!==e[0].dataType&&6!==e[0].dataType&&12!==e[0].dataType)throw Error("Tile only support float, float16, int32, and uint32 data types");if(7!==e[1].dataType)throw Error("Tile `repeats` input should be of int64 data type");if(1!==e[1].dims.length)throw Error("Tile `repeats` input should be 1-D");if(nh(e[1]).length!==e[0].dims.length)throw Error("Tile `repeats` input should have same number of elements as rank of input data tensor")})(e.inputs),e.compute((i=(r=e.inputs)[0].dims,n=((e,r)=>{let i=[];for(let a=0;a<e.length;++a)i.push(e[a]*r[a]);return i})(i,a=(void 0)??nh(r[1])),s=e3.size(n),o=r[0].dataType,u=tO("input",o,i.length),l=tR("output",o,n.length),{name:"Tile",shaderCache:{hint:`${a}`,inputDependencies:["rank"]},getRunData:()=>({outputs:[{dims:n,dataType:r[0].dataType}],dispatchGroup:{x:Math.ceil(s/64)},programUniforms:[{type:12,data:s},...tS(r[0].dims,n)]}),getShaderSource:e=>`
